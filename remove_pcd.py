...
def main():
  ...
  batch = np.random.binomial(1, train[img_idxs])
    ...
    while True:
      ...
      if rc + FLAGS.mul_std * std < FLAGS.cut_thre:
        ...
        if epoch % FLAGS.save_per == 0:
          ...
          img_idxs = np.random.choice(len(train), FLAGS.n_save_imgs)
          imgs = np.random.binomial(1, train[img_idxs])
          imgs = reconst(b, c, w, imgs)
